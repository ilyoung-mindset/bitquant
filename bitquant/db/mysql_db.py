...
class Mysql(object):
    ...
    def __getConn():
        ...
        '''
        db = pymysql.connect(host=_params['host'], 
                             port=3306, 
                             user='bitquant_test', 
    ...
    def _createTable(self, table, attr_dict, constraint):
        ...
        logging.debug('_createTable:'+sql)
    ...
    def _insertDic(self, table, attrs):
        ...
        value_str = self._transferContent(
            attrs.values())  # ','.join(attrs.values())
        values_sql = ' values(' + value_str + ')'
        sql = 'insert into %s' % table
        sql = sql + attrs_sql + values_sql
        logging.debug( '_insert:'+sql)
    ...
    def _insertMany(self, table, attrs, values):
        ...
        try:
            ...
            self._conn.rollback()
            error = '_insertMany executemany failed! ERROR (%s): %s' % (
                e.args[0], e.args[1])
            logging.error(error)
            return 0
