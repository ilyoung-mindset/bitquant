...
class HuobiREST:
    def __init__(self, acccess_key=None, secret_key=None, market_url=None, trade_url=None, params={}):
        
        if not params.__contains__('http_proxy_host'):
            params = {'http_proxy_host': None}
    ...
    def get_accounts(self):
        ...
        accounts = self.api_key_get(params, path)
    ...
    def send_order(self, amount, source, symbol, _type, price=0):
        ...
        try:
            ...
            logging.error('get acct_id error.%s' % e)
    ...
    def http_get_request(self, url, params, add_to_headers=None):
        ...
        try:
            ...
            logging.error("httpGet failed, detail is:%s,%s" % (response.text, e))
            return
    ...
    def http_post_request(self, url, params, add_to_headers=None):
        ...
        try:
            ...
            logging.error("httpPost failed, detail is:%s,%s" % (response.text, e))
            return
    ...
    def api_key_get(self, params, request_path):
        ...
        params.update({'AccessKeyId': self.acccess_key,
    ...
    def api_key_post(self, params, request_path):
        method = 'POST'
        timestamp = datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S')
        params_to_sign = {'AccessKeyId': self.acccess_key,
                        'SignatureMethod': 'HmacSHA256',
                        'SignatureVersion': '2',
                        'Timestamp': timestamp}
